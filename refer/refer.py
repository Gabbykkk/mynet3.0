...
class REFER:
    ...
    def __init__(self, data_root, dataset='refcoco', splitBy='unc'):
        ...
        # load refs from data/dataset/refs(dataset).json  加载引用对象的数据文件
        tic = time.time()
        ...
        # load annotations from data/dataset/instances.json  加载图像和注释信息
...
if __name__ == '__main__':
    ...
    ref_ids = refer.getRefIds()  # 获取数据集中所有引用对象的ID

    ref_ids = refer.getRefIds(split='train')  # 获取训练集中引用对象的ID
    print('There are %s training referred objects.' % len(ref_ids))  # 打印训练集中引用对象的数量
