...
class CollectDiffuseAttention(nn.Module):
    ...
    def __init__(self, temperature, attn_dropout=0.1, margin=0.2):
        super().__init__()
    ...
    def forward(self, q, kc, kd, v, mask=None):
        ...
        attn_dif = torch.bmm(kd, q.transpose(1, 2))  # n*b,h*w,1  注意力特征Adif,attention map  过将键值对 kd 与查询向量 q 的转置进行点乘得到的
        ...
        # 计算对比学习损失
        if self.training:
            loss = self.contrastive_loss_ou(attn_col_logit,attn_dif_logit)
            return output, loss
        else:
            return output

    def contrastive_loss_ou(self, attn_col_logit, attn_dif_logit):
        # 计算欧氏距离
        distance = torch.norm(attn_col_logit - attn_dif_logit, dim=2)
        # 计算相似度
    ...
    def contrastive_loss_cos(self, attn_col_logit, attn_dif_logit):
        # 计算余弦相似度
        similarity = F.cosine_similarity(attn_col_logit, attn_dif_logit, dim=2)
        # 通过余弦相似度计算对比学习损失
        loss = (1 - similarity).mean()
        return loss
...
class GaranAttention(nn.Module):
    ...
    def forward(self, q, v, mask=None):
        ...
        if self.training:
            output, con_loss = self.attention(q, kc, kd, v)
        else:
            output = self.attention(q, kc, kd, v)
