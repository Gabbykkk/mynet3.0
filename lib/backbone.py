...
# from add.MC import MutualComplementarity
# from add.SELayer import SELayer
# from add.findit import SimpleAttention
from add.dynamic_attention.query_dynamic_attention import QueryDynamicAttention
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B C Wh Ww [20,96,80,80]
        if self.norm is not None:
            Wh, Ww = x.size(2), x.size(3)
            x = x.flatten(2).transpose(1, 2)  # [20,6400,96]
            x = self.norm(x)  # [20,6400,96]
            x = x.transpose(1, 2).view(-1, self.embed_dim, Wh, Ww)  # [20,96,80,80]
...
class MultiModalSwinTransformer(nn.Module):
    def __init__(self,
        ...
        # 将输入图像划分为非重叠的图像块，并进行嵌入，得到图像的特征表示
        ...
        # build layers
        # layers是包含多个MMBasicLayer模块的模块列表，每一个MMBasicLayer模块由一系列swinTransformer块组成，用于提取和融合特征
        ...
        # num_features：每个层的输出特征维度列表
        num_features = [int(embed_dim * 2 ** i) for i in range(self.num_layers)]
        self.num_features = num_features

        # add a norm layer for each output
        for i_layer in out_indices:
            layer = norm_layer(num_features[i_layer])  # norm_layer：对每个输出进行归一化的模块
    ...
    def forward(self, x, l, l_mask):
        """Forward function."""
        x = self.patch_embed(x)  # [B,C,H,W ]  [20,96,80,80]
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.fusion = PWAM(dim,  # both the visual input and for combining, num of channels
                           ...
                           dropout=fusion_drop)
        # findit--simpleAttention
        # self.findit = SimpleAttention(dim, 768, dim)
        # QRNet-attention
        ...
        # mc
        # self.mc = MutualComplementarity(dim,dim)
        # se layer
    ...
    def forward(self, x, H, W, l_feats, l_mask):
        """ Forward function.
        ...
        # PWAM fusion  原
        x_residual = self.fusion(x, l_feats, l_mask)

        # findit fusion
        # x_residual = self.findit(x, l_feats, H, W)

        # QRNet
        # x:[20,6400,96] ; l_pooler:[20,768]
        #####  garan使用l_feats时
        # l = torch.mean(l_feats, dim=2)
        #####
        # x = self.qdatt(x, l)  # [20,6400,96]

        # ===mc
        # x = self.mc(x,x_residual,H,W)
        # ===se layer
        # x = x + self.selayer( x_residual, H, W )

        # apply a gate on the residual  源代码
