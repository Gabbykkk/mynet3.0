...
class PWAM(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        lang = self.image_lang_att(x, l, l_mask)  # (B, H*W, dim)  lang 就是注意力图（Attention Map），它表示了视觉特征 x 和语言特征 l 之间的注意力权重分配情况
...
class SpatialImageLanguageAttention(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        #  sim_map 表示了每个视觉特征与语言特征之间的注意力权重分布,Attention Map
        sim_map = torch.matmul(query, key)  # (B, self.num_heads, H*W, N_l)  对查询（query）和键（key）进行点积操作得到 sim_map
        sim_map = (self.key_channels ** -.5) * sim_map  # scaled dot product  对 sim_map 进行缩放处理，以确保数值稳定性

        sim_map = sim_map + (1e4 * l_mask - 1e4)  # assign a very small number to padding positions 对填充位置进行掩码操作，将填充位置的注意力权重设置为一个极小的数值
        sim_map = F.softmax(sim_map, dim=-1)  # (B, num_heads, h*w, N_l)  对 sim_map 进行 softmax 操作，将注意力权重归一化到 [0, 1] 范围内
