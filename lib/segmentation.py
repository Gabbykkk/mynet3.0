...
__all__ = ['lavt', 'lavt_one']  # __all__列表指定了在使用from segmentation import *语法时要导入的模块或函数的名称


# segmentation定义了两个模型，分别是：
# lavt:
# lavt_one:

# LAVT
# 根据给定的参数初始化了Swin Transformer的主干网络，并创建了LAVT模型实例
def _segm_lavt(pretrained, args):
    ...
    # 创建了一个SimpleDecoding类和一个LAVT类的实例
    model_map = [SimpleDecoding, LAVT]

    classifier = model_map[0](8*embed_dim, 768)   # c4_dims = 8*embed_dim, l_dim=768
    base_model = model_map[1]

    model = base_model(backbone, classifier)
    return model  # 返回LAVT模型的实例
...
###############################################
# LAVT One: put BERT inside the overall model #############不用lavt_one
###############################################
# 根据给定的参数初始化了Swin Transformer的主干网络，并创建了LAVT_one模型实例
